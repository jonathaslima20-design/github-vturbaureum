        ...
        <div className="px-4 pt-4 pb-0">
          ...
          <div className="px-5">
            ...
            <div className="relative -mt-20 mb-4 flex flex-col items-center">
              ...
              {/* Bio */}
              {config.bio && (
                <p className="mt-4 text-center text-gray-500 max-w-[320px]">
                  {config.bio}
                </p>
              )}
              ...
              {config.social_buttons && config.social_buttons.length > 0 && (
                <div className="mt-6 flex items-center gap-4">
                  {config.social_buttons.map((btn, i) => (
                    <div
                      key={i}
                      className="h-14 w-14 rounded-full border border-gray-200 flex items-center justify-center bg-white"
                    >
                      <SocialIcon type={btn} />
                    </div>
                  ))}
        ...
        <div className="px-4 mt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            {products.slice(0, 4).map((product, i) => (
              <ProductCardReplica key={i} product={product} accent={config.accent_color} />
            ))}
          </div>
        </div>
      </div>
...
function ProductCardReplica({ product, accent }: { product: StorefrontProduct; accent?: string }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow overflow-hidden flex flex-col">
      ...
      <div className="p-2 flex-1 flex flex-col">
        <h3 className="font-semibold text-xs leading-tight mb-2 line-clamp-2 min-h-[32px] text-gray-900">
          {product.title}
        </h3>
        <div className="mt-auto">
          {hasDiscount ? (
            <div className="space-y-0.5">
              ...
              <div className="text-sm font-bold" style={{ color: accent || '#0f172a' }}>
...
function SocialIcon({ type }: { type: string }) {
  const cls = "h-6 w-6 text-gray-700";
